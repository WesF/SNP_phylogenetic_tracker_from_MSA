...
class State(object):
	...
	def set_working_dir(self, directory):
		...
		self.input = os.path.join(self.working_dir, 'input.fasta')
		self.msa = os.path.join(self.working_dir, 'msa.fasta')
		self.position_data = os.path.join(self.working_dir, 'position_data.txt')
		self.temp = os.path.join(self.working_dir, 'temp.fasta')
		self.blast_xml =  os.path.join(self.working_dir, 'blast.xml')
		self.blastdb = os.path.join(self.working_dir, 'blastdb')
		self.mutations_to_krona = os.path.join(self.working_dir, 'krona_input_mutations.txt')
		self.mutations_from_krona = os.path.join(self.working_dir, 'krona_mutation_distributions.html')
		self.to_krona = os.path.join(self.working_dir, 'krona_input.txt')
		self.from_krona = os.path.join(self.working_dir, 'krona_phylogeny.html')
		self.needle_out = os.path.join(self.working_dir, 'needle_out.fasta')
...
class CmdLineParser(object):
	parsed=None
	def __init__(self):
		self.parser = argparse.ArgumentParser(description='Resistance project tool')
		subparsers = self.parser.add_subparsers(help='sub-command help')
		parser_new = subparsers.add_parser('new', help='start process on new metagenomic data')
		parser_new.set_defaults(mode='new')
		parser_new.add_argument('-t', '--to_step', type=int, default='999', help="run till this step")
		parser_new.add_argument('input', type=str, help="input a fasta metagenome file") 
	...
	def parse(self,state):
		...
		if self.parsed.mode == 'new':
			...
			if self.parsed.from_step:
				...
				self.from_step = get_last_successful_step(state)+1
				logging.info('continue from after last successful step - %d' %(self.from_step))
			self.to_step = min(int(self.parsed.to_step), len(STEPS))

def get_last_succesful_step(state):
	file_path = state.last_state
	with file(file_path, 'r') as f:
		return int(f.read())

def set_last_successful_step(state, step):
	file_path = state.last_state
...
def import_new_sequence(state,input_file_path):
	...
	output_folder = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'results', str(datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")))
	os.mkdir(output_folder)
	state.set_working_dir(output_folder)
	set_last_successful_step(state,0)
	#copy input file
	shutil.copyfile(input_file_path, state.input)
...
def set_file_logger(state):
	...
	formatter = logging.Formatter('%(asctime)-12s: %(levelname)-8s %(message)s',  datefmt='%d/%m/%Y_%H:%M:%S')
...
def main(argv):
	...
	if parser.mode =='continue':
		...
		set_last_successful_step(state,parser.from_step-1)
	...
	for step in xrange(parser.from_step, parser.to_step+1):
		...
		set_last_successful_step(state,step)
